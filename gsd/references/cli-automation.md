<overview>
**Core principle:** If it has a CLI or API, the LLM does it. Never ask the human to perform manual steps that the LLM can automate.

This reference documents what the LLM CAN and SHOULD automate during plan execution.
</overview>
...
**Critical distinction:** When the LLM tries to use a CLI/API and gets an authentication error, this is NOT a failure - it's a gate that requires human input to unblock automation.

**Pattern: LLM encounters auth error → creates checkpoint → you authenticate → LLM continues**
...
<!-- After authentication, LLM retries automatically -->
...
<!-- After key provided, LLM writes to .env and continues -->
...
**When the LLM encounters authentication error during execution:**
...
**Key difference:** The LLM does EVERYTHING possible first (account creation, API requests), only asks human for the one thing with no automation path.
</required_cases>

<quick_reference>

| Action | CLI/API? | LLM does it? |
|--------|----------|-----------------|
...
**Default answer: YES.** Unless explicitly in the "NO" category, the LLM automates it.
</quick_reference>
...
**The rule:** If the LLM CAN do it, the LLM MUST do it.

Checkpoints are for:
- **Verification** - Confirming the LLM's automated work looks/behaves correctly
...
**This keeps the agentic coding workflow intact - the LLM does the work, you verify results.**
</summary>
