...
This is CONTEXT INJECTION so the LLM implements correctly.
...
- Formats output for the LLM to consume during implementation

**Quality bar:** Will this context make the LLM generate correct, modern code?

**OpenCode Integration:** Use the Task tool with `subagent_type: "general"` to spawn research subagents. For documentation lookups, subagents should use the Context7 MCP server when available.
</overview>

<stack_subagent_prompt>
## Stack Research Subagent

Use this template for spawning stack research subagents via the Task tool:
...
**This research is for LLM implementation, not humans.**

You are providing context injection so the LLM implements correctly. The LLM's training data may have outdated library versions, deprecated APIs, or old patterns. Your research overrides stale knowledge with current, accurate information.

**Quality bar:** Only include information that will make the LLM generate correct, modern code.
...
1. What library/tool should be used?
...
Use Context7 MCP for library documentation when available:
- mcp__context7__resolve-library-id to find library
- mcp__context7__get-library-docs for current documentation
...
**Purpose:** Libraries and tools for implementation
**Researched:** [date]
...
Use this template for spawning implementation research subagents via the Task tool:
...
The LLM may generate outdated code patterns. Your job is to provide:
- Current API syntax (not deprecated alternatives)
- Working code examples (not pseudocode)
- "Do this, not that" corrections

**Quality bar:** The LLM reads this file, then generates correct modern code.
...
2. What code does the LLM need to generate correctly?
3. What deprecated patterns might the LLM default to?
...
Use WebSearch for current documentation (2024-2025).
...
**Purpose:** Current API patterns for implementation
**Researched:** [date]
...
```[language]
// LLM may generate this - it's outdated
...
Use this template for spawning risks research subagents via the Task tool:

```
<subagent_prompt>
## Objective

Research what the LLM might get WRONG when implementing this project.
...
The LLM's training data includes outdated tutorials, deprecated APIs, and bad patterns. Your job is to identify:
- What the LLM might generate incorrectly
- Deprecated patterns to explicitly avoid
- Common implementation mistakes in this domain

**Quality bar:** After reading this, the LLM avoids specific mistakes.
...
1. What deprecated patterns might the LLM default to?
...
Use Context7 MCP for library documentation to identify deprecated patterns.
...
**Purpose:** What the LLM might get wrong
**Researched:** [date]
...
In OpenCode, spawn research subagents using the Task tool. Launch all 3 subagents in a SINGLE message for parallel execution:
