<overview>
LLM-executable plans have a specific format that enables the LLM to implement without interpretation. This reference defines what makes a plan executable vs. vague.

**Key insight:** PLAN.md IS the executable prompt. It contains everything the LLM needs to execute the phase, including objective, context references, tasks, verification, success criteria, and output specification.
</overview>

<core_principle>
A plan is LLM-executable when the LLM can read the PLAN.md and immediately start implementing without asking clarifying questions.

If the LLM has to guess, interpret, or make assumptions - the task is too vague.
</core_principle>
...
<type name="auto">
**Default task type** - LLM executes autonomously.
...
Use for: Everything the LLM can do independently (code, tests, builds, file operations).
</type>

<type name="checkpoint:human-action">
**RARELY USED** - Only for actions with NO CLI/API. LLM automates everything possible first.
...
**Execution:** LLM automates everything with CLI/API, stops only for truly unavoidable manual steps.
</type>

<type name="checkpoint:human-verify">
**Human must verify LLM's work** - Visual checks, UX testing.
...
**Execution:** LLM builds the feature, stops, provides testing instructions, waits for approval/feedback.
</type>
...
**Execution:** LLM presents options with balanced pros/cons, waits for decision, proceeds with chosen direction.
</type>

**When to use checkpoints:**

- Visual/UX verification (after LLM builds) → `checkpoint:human-verify`
...
- Anything with CLI/API (LLM automates it) → `type="auto"`
...
**Golden rule:** If the LLM CAN automate it, the LLM MUST automate it.

See `@~/.config/opencode/gsd/references/checkpoints.md` for comprehensive checkpoint guidance.
</task_types>
...
Reference files that the LLM needs to understand before implementing.
</context_references>
...
LLM: "How? What type? What library? Where?"
</too_vague>
...
LLM can implement this immediately.
</just_right>

<too_detailed>
Writing the actual code in the plan. Trust the LLM to implement from clear instructions.
</too_detailed>
</specificity_levels>
...
These require the LLM to decide WHAT to do. Specify it.
</vague_actions>
...
The LLM doesn't know your standards. Be explicit.
</missing_context>
</anti_patterns>

<sizing_tasks>
Good task size: 15-60 minutes of LLM work.
...
**Note on scope:** If a phase has >3 tasks or spans multiple subsystems, split into multiple plans using the naming convention `{phase}-{plan}-PLAN.md`. See `@~/.config/opencode/gsd/references/scope-estimation.md` for guidance.
</sizing_tasks>
