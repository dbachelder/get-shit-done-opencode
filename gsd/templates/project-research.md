...
Template for `.planning/research/{category}.md` - implementation context for OpenCode.
...
**Context injection for OpenCode implementation quality.**

This is NOT research for humans to make decisions.
This is context that the LLM reads during implementation to generate correct, modern code.

**Quality bar:** Will this make the LLM generate better code?
...
For library/framework documentation, use Context7 MCP which provides current, authoritative documentation:

```
# Resolve library ID first
mcp__context7__resolve-library-id topic="prisma orm"

# Then fetch specific documentation
mcp__context7__get-library-docs libraryId="/prisma/prisma" topic="migrations"
```

**Fallback: Web Search + Official Docs**

When Context7 doesn't index a library:
1. Search for official documentation
2. Verify findings against official sources
3. Mark confidence level accordingly

**Source Priority:**
1. Context7 MCP (HIGH confidence - current, authoritative)
2. Official documentation (HIGH confidence)
3. Web search verified with official docs (MEDIUM confidence)
4. Web search alone (LOW confidence - mark for validation)
</research_tools>
...
**Purpose:** Libraries and tools for OpenCode to use during implementation
**Researched:** [date]
...
**Context7:** [library ID if available, e.g., "/tanstack/react-query"]
...
**Purpose:** Current API patterns for OpenCode to use
**Researched:** [date]
...
```[language]
// The LLM may generate this - it's outdated
[old pattern]
...
**Notes:** [Implementation considerations the LLM should know]
...
**What happens:** [The incorrect approach the LLM might take]
...
1. **[Most important thing the LLM must not mess up]**
...
- Anything that doesn't directly improve the LLM's implementation
