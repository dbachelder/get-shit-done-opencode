...
During plan execution, use OpenCode's native task tracking alongside STATE.md:
- At plan start: Create todos for each task in the plan
- During execution: Mark todos as in_progress/completed
- This provides real-time visibility into execution progress

STATE.md remains the cross-session memory; todowrite is for session-level tracking.
</opencode_integration>
...
1. Proceed anyway - Issues won't block this phase
2. Address first - Let's resolve before continuing
3. Review previous - Show me the full summary
...
1. Create .planning/ISSUES.md if doesn't exist (use `~/.config/opencode/gsd/templates/issues.md`)
...
**Critical: The LLM automates everything with CLI/API before checkpoints.** Checkpoints are for verification and decisions, not manual work.
...
```
I automated: [what the LLM already did via CLI/API]
...
[N] issues were logged during this phase. Review now?

1. Review issues - Analyze with /gsd/consider-issues
2. Continue - Address later, proceed to next work

Reply with a number.

**If "Review issues" selected:**
- Next, run `/gsd/consider-issues`
