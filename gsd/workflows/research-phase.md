...
Triggered by /gsd/research-phase command when the domain is niche, complex, or the LLM's training is likely stale.
...
<when_to_use>
**This workflow is for domains where the LLM fails without research:**
...
- Specialized frameworks with active ecosystems the LLM may not know
...
- What's SOTA vs what the LLM thinks is SOTA?
...
- What version is current? (the LLM's training may be stale)
...
- Key changes since [LLM's training]: [from docs/WebSearch]
...
**From ~/.config/opencode/gsd/references/research-pitfalls.md:**
...
- [ ] Checked for libraries the LLM might not know about
...
This produces higher quality plans because the LLM knows:
