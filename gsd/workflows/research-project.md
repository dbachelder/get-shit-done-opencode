<purpose>
Research implementation context for OpenCode before roadmap creation.

This is NOT research for human decision-making.
This is CONTEXT INJECTION so OpenCode implements correctly.

Research quality directly impacts implementation quality.
The LLM's training data may have outdated APIs, deprecated patterns, old syntax.
...
OpenCode will read these files during implementation. The research should:
- Override outdated patterns the LLM might default to
- Provide current API syntax and code examples
- Explicitly correct common mistakes the LLM makes
...
**Include if:** High confidence, directly actionable, will improve the LLM's implementation
...
This provides current, accurate documentation that overrides stale training data.
</research_tools>

<required_reading>
**Read before executing:**
1. `~/.config/opencode/gsd/references/research-subagent-prompts.md` - Prompt templates
2. `~/.config/opencode/gsd/templates/project-research.md` - Output format
</required_reading>
...
3. **risks.md** - What the LLM might get wrong
   - Deprecated patterns the LLM may default to
...
Read prompt templates from `~/.config/opencode/gsd/references/research-subagent-prompts.md`
...
**NOT:**
```[language]
// Deprecated - the LLM may generate this
