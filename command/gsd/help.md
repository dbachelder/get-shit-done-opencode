...
This installs GSD commands to `~/.config/opencode/command/gsd/` and skill files to `~/.config/opencode/gsd/`.
...
See what the LLM is planning to do before it starts.

- Shows the LLM's intended approach for a phase
- Lets you course-correct if assumptions are wrong
...
- Uses OpenCode's native todowrite/todoread for task tracking
- Creates SUMMARY.md after completion
- Updates STATE.md with accumulated context
- Uses Task tool to spawn subagents for autonomous execution
...
GSD leverages OpenCode's native capabilities:

- **Task tool** - Spawns subagents for parallel research and autonomous execution
- **todowrite/todoread** - Tracks task progress during plan execution
- **Context7 MCP** - Documentation lookups during research phases
