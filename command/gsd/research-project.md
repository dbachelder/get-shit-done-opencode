...
This is NOT research for human decision-making. This is context injection so the LLM can implement correctly with current APIs, patterns, and best practices.
...
- **Risks** - What the LLM might get wrong, deprecated patterns to avoid

Each subagent writes directly to `.planning/research/` preserving main context.
</objective>

<execution_context>
@{{GSD_PATH}}/workflows/research-project.md
@{{GSD_PATH}}/templates/project-research.md
@{{GSD_PATH}}/references/research-subagent-prompts.md
</execution_context>
...
If PROJECT.md is too vague for specific research targets:

## Research Scope

What specific implementation questions should we research?

1. [Option based on detected domain]
2. [Option based on detected domain]
3. Let me explain
...
2. Use the Task tool to spawn subagents in parallel (all at once if ≤3):
   - subagent_type: "general"
   - **stack.md** - Libraries/tools for each feature in Scope
   - **implementation.md** - Current API patterns and code examples
   - **risks.md** - What the LLM might get wrong, deprecated patterns
